# =====================================================
# 🚨 PROTECTION CONFIGURATION CENTRALISÉE 🚨
# =====================================================
# ATTENTION : Ce fichier utilise la configuration centralisée
# 
# ❌ INTERDIT :
...
# =====================================================
# CONFIGURATION CENTRALISÉE (SEULE SOURCE)
# =====================================================

def get_centralized_config():
    """Récupère la configuration centralisée - SEULE FONCTION AUTORISÉE"""
    try:
        config = get_agent_config()
        logger.info("✅ Configuration centralisée chargée")
        return config
...
# Log de la configuration centralisée
logger.info("🔍 DIAGNOSTIC: Configuration centralisée")
logger.info(f"   LiveKit URL: {CENTRALIZED_CONFIG['livekit']['url']}")
logger.info(f"   Agent Port: {CENTRALIZED_CONFIG['port']}")
logger.info(f"   Mistral URL: {CENTRALIZED_CONFIG['services']['mistral']}")
logger.info(f"   Vosk URL: {CENTRALIZED_CONFIG['services']['vosk']}")

# URLs des services depuis la configuration centralisée
MISTRAL_API_URL = CENTRALIZED_CONFIG['services']['mistral']
VOSK_STT_URL = CENTRALIZED_CONFIG['services']['vosk']
...
class RobustLiveKitAgent:
    ...
    async def create_robust_tts(self):
        ...
        # Note: OpenAI API key reste dans .env pour la sécurité
